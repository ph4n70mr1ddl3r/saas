...
async fn main() -> anyhow::Result<()> {
    tracing_setup::init("saas-erp-fixed-assets");
    jwt::init_jwt_secret();

    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/fixed-assets.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8023".into()).parse()?;
    ...
    let bus = NatsBus::connect(&nats_url, "saas-erp-fixed-assets").await?;
    ...
    tracing::info!("Fixed Assets service listening on port {}", port);
