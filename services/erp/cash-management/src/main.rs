...
async fn main() -> anyhow::Result<()> {
    ...
    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/cash-management.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8024".into()).parse()?;
    ...
    let bus = NatsBus::connect(&nats_url, "saas-erp-cash-management").await?;
