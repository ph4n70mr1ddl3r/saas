...
async fn main() -> anyhow::Result<()> {
    tracing_setup::init("saas-erp-expense-mgmt");
    jwt::init_jwt_secret();

    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/expense-mgmt.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8025".into()).parse()?;
    ...
    let bus = NatsBus::connect(&nats_url, "saas-erp-expense-mgmt").await?;
    ...
    tracing::info!("Expense Management service listening on port {}", port);
