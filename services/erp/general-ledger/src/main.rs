...
async fn main() -> anyhow::Result<()> {
    tracing_setup::init("saas-erp-general-ledger");
    jwt::init_jwt_secret();

    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/general-ledger.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8020".into()).parse()?;
    ...
    let bus = NatsBus::connect(&nats_url, "saas-erp-general-ledger").await?;
    ...
    tracing::info!("General Ledger service listening on port {}", port);
