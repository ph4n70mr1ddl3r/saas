...
impl InventoryService {
    ...
    pub async fn create_stock_movement(&self, input: CreateStockMovement) -> AppResult<StockMovementResponse> {
        ...
        if input.movement_type == "receipt" {
            ...
            let _ = self.bus.publish("scm.inventory.stock.received", saas_proto::events::StockReceived {
    ...
    pub async fn create_reservation(&self, input: CreateReservation) -> AppResult<ReservationResponse> {
        input.validate().map_err(|e| saas_common::error::AppError::Validation(e.to_string()))?;
        let reservation = self.reservation_repo.create(&input).await?;
        self.stock_level_repo.reserve(&input.item_id, &input.warehouse_id, input.quantity).await?;
        let _ = self.bus.publish("scm.inventory.stock.reserved", saas_proto::events::StockReserved {
            item_id: input.item_id.clone(),
