use crate::models::warehouse::{CreateWarehouse, UpdateWarehouse, WarehouseResponse};
...
impl WarehouseRepo {
    ...
    pub async fn create(&self, input: &CreateWarehouse) -> AppResult<WarehouseResponse> {
        ...
        sqlx::query("INSERT INTO warehouses (id, name, address) VALUES (?, ?, ?)")
            .bind(&id)
            .bind(&input.name)
            .bind(&input.address)
            .execute(&self.pool)
    ...
    pub async fn update(&self, id: &str, input: &UpdateWarehouse) -> AppResult<WarehouseResponse> {
        ...
        sqlx::query("UPDATE warehouses SET name=?, address=?, is_active=? WHERE id=?")
