...
#[derive(Clone)]
pub struct StockLevelRepo { pool: SqlitePool }
...
impl StockLevelRepo {
    ...
    pub async fn get_by_item_warehouse(&self, item_id: &str, warehouse_id: &str) -> AppResult<Option<StockLevelResponse>> {
    ...
    pub async fn upsert_receipt(&self, item_id: &str, warehouse_id: &str, quantity: i64) -> AppResult<StockLevelResponse> {
        ...
        self.get_by_item_warehouse(item_id, warehouse_id).await?
            .ok_or_else(|| AppError::Internal("Failed to upsert stock level".into()))
    }

    pub async fn reserve(&self, item_id: &str, warehouse_id: &str, quantity: i64) -> AppResult<StockLevelResponse> {
        let id = uuid::Uuid::new_v4().to_string();
        sqlx::query(
