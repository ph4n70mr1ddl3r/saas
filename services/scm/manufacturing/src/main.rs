...
async fn main() -> anyhow::Result<()> {
    tracing_setup::init("saas-scm-manufacturing");
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/manufacturing.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8033".into()).parse()?;
    ...
    tracing::info!("Manufacturing service listening on port {}", port);
