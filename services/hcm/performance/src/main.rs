...
use routes::AppState;
use service::PerformanceService;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    tracing_setup::init("saas-hcm-performance");
    jwt::init_jwt_secret();

    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/performance.db".into());
    let nats_url = env::var("NATS_URL").unwrap_or_else(|_| "nats://localhost:4222".into());
    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8015".into()).parse()?;
    ...
    let bus = NatsBus::connect(&nats_url, "saas-hcm-performance").await?;

    let service = PerformanceService::new(pool, bus.clone());
    let cors_origin =
