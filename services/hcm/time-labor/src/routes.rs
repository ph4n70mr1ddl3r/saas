...
pub fn build_router(state: AppState) -> Router {
    Router::<AppState>::new()
        ...
        .route(
            "/api/v1/timesheets/employee/{employee_id}",
            get(handlers::list_timesheets_by_employee),
        )
        .route(
            "/api/v1/timesheets/{id}/submit",
            axum::routing::put(handlers::submit_timesheet),
        )
        .route(
        ...
        .route(
            ...
            axum::routing::put(handlers::approve_leave_request),
        )
        .route(
            "/api/v1/leave/requests/{id}/reject",
            axum::routing::put(handlers::reject_leave_request),
        )
        .route(
            "/api/v1/leave/balances/employee/{employee_id}",
            get(handlers::list_leave_balances_by_employee),
        )
        .route(
            "/health",
