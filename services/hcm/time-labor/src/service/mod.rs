...
impl TimeLaborService {
    ...
    pub async fn submit_timesheet(&self, id: &str) -> AppResult<Timesheet> {
        ...
        if ts.status != "draft" {
            return Err(AppError::Validation(
                format!("Timesheet '{}' must be in draft status to submit (current: {})", id, ts.status)
            ));
        }
    ...
    pub async fn approve_timesheet(&self, id: &str) -> AppResult<Timesheet> {
        ...
        if ts.status != "submitted" {
            return Err(AppError::Validation(
                format!("Timesheet '{}' must be in submitted status to approve (current: {})", id, ts.status)
            ));
        }
    ...
    pub async fn approve_leave_request(&self, id: &str) -> AppResult<LeaveRequest> {
        ...
        if req.status != "pending" {
            return Err(AppError::Validation(
                format!("Leave request '{}' must be pending to approve (current: {})", id, req.status)
            ));
        }

        // Check leave balance before approving
        let days = calculate_leave_days(&req.start_date, &req.end_date);
        let balance = self.repo.get_leave_balance(&req.employee_id, &req.leave_type).await?;
        if balance.remaining < days {
            return Err(AppError::Validation(
                format!("Insufficient leave balance: have {:.1} days, requested {:.1} days", balance.remaining, days)
            ));
        }

        let result = self.repo.update_leave_request_status(id, "approved").await?;
        // Deduct from leave balance -- propagate error instead of discarding
        self.repo.deduct_leave_balance(&result.employee_id, &result.leave_type, days).await?;
        Ok(result)
    }
    ...
    pub async fn reject_leave_request(&self, id: &str) -> AppResult<LeaveRequest> {
        ...
        if req.status != "pending" {
            return Err(AppError::Validation(
                format!("Leave request '{}' must be pending to reject (current: {})", id, req.status)
            ));
        }
    ...
    pub async fn handle_employee_created(&self, employee_id: &str) -> anyhow::Result<()> {
        ...
        for (leave_type, entitled) in defaults {
            self.repo.create_leave_balance(employee_id, leave_type, entitled).await?;
        }
        Ok(())
    }
}
...
fn calculate_leave_days(start_date: &str, end_date: &str) -> f64 {
    ...
    match (start, end) {
        (Ok(s), Ok(e)) => {
            ...
            if diff > 0 { diff as f64 } else { 1.0 }
        }
        _ => 1.0,
    }
}
